...
function kitTagPattern(kitText: string): RegExp | null {
  ...
  if (/meta[-\s]?b.*hypothyroid/.test(text)) return /metabolic|obes|weight|sedentary|pre.?diabet|diabet|insulin|hypothyroid|thyroid|age above 40|over 40|>\s*40/i;
  if (/meta[-\s]?b.*pcos/.test(text)) return /pcos|pcod|metabolic|obes|weight|sedentary|pre.?diabet|diabet|insulin|irregular period/i;
  if (/meta[-\s]?b.*post/.test(text)) return /post.?menopause|menopause|metabolic|obes|weight|sedentary|pre.?diabet|diabet|insulin/i;
  if (/meta[-\s]?b/.test(text)) return /metabolic|obes|weight|sedentary|pre.?diabet|diabet|insulin|pcos|hypothyroid|hyperthyroid|thyroid|age above 40|over 40|>\s*40/i;
  ...
  if (/\bfphl\b|female pattern/.test(text)) return /female|androgen|genetic|family|ludwig|pattern|bodybuild|heavy gym|gym/i;
  if (/\bmphl\b|male pattern/.test(text)) return /male|androgen|genetic|family|norwood|pattern|dht|receding|crown|vertex|temple|bodybuild|heavy gym|gym/i;
